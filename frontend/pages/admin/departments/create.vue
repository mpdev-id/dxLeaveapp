...
<template>
  <div>
    <h1 class="text-3xl font-bold mb-4">Create Department</h1>
    <form @submit.prevent="createDepartment" class="w-full max-w-lg">
...
<script setup>
  ...
  const createDepartment = async () => {
    try {
      await useFetch('/api/admin/master/departments', {
        method: 'POST',
        body: form.value,
        headers: {
